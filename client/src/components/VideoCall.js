...
export default function VideoCall() {
  const {
      ...
      peer, isVideoCall, answerCall, rejectCall, endCall
  } = useCall()
  // ConversationsProvider never exposed `contacts`, so this was always
  // undefined and every call showed the raw peer id instead of a name.
  ...
  }, [localStream, isVideoCall, callActive])

  useEffect(() => {
    // Separate refs per element: both the <video> and the hidden <audio>
    // previously shared one ref, so whichever mounted last won and the other
    // never received the stream — audio calls came through silent.
    if (remoteVideoRef.current && remoteStream) remoteVideoRef.current.srcObject = remoteStream
    if (remoteAudioRef.current && remoteStream) remoteAudioRef.current.srcObject = remoteStream
  }, [remoteStream, isVideoCall, callActive])

  // Call timer starts when media actually flows.
  useEffect(() => {
    if (!remoteStream) { setElapsed(0); return }
  ...
  const peerId = (peer && peer.id) || (incomingCall && incomingCall.callerId) || (outgoingCall && outgoingCall.targetId)
  const match = contacts.find(c => c.id === peerId)
  const targetName = (match && match.name) || peerId || 'Unknown'
  ...
  return (
    <div style={{
      ...
      {callActive && (
          <>
             {/* Remote audio is always mounted so voice calls are audible
                 regardless of which layout is showing. */}
             <audio ref={remoteAudioRef} autoPlay style={{ display: 'none' }} />

             {remoteStream ? (
                isVideoCall ? (
                    <video ref={remoteVideoRef} autoPlay playsInline data-testid="remote-video" style={{ width: '100%', height: '100%', objectFit: 'cover', position: 'absolute', top: 0, left: 0 }} />
                ) : (
                    <div style={{ textAlign: 'center', zIndex: 10 }}>
                        ...
                        <h2>{targetName}</h2>
                        <p style={{ color: '#00a884' }} data-testid="call-timer">{formatDuration(elapsed)}</p>
                    </div>
                )
             ) : (
